	...
	{		
		for (Dataset file : getFiles())
		{
			...
			{	
				...
				drawLine(graphics, calculateColor(file, 1, progress), width, height, stroke, font, x1, y1, x2, y2);
			}
			...
			for (Entry<Dataset, Integer> entry : marker.entrySet())
			{
				...
				if (hover)
				{
					if (series.containsKey(file))
					{
						...
						drawMarker(graphics, calculateColor(file, 0.5, Math.pow(0, 10)), width, height, stroke, font, domain, range);
					}
				}
				else
				{
		...
		if (selected != null)
		{
			...
			case MEAN_STDEV:
				...
				drawLine(graphics, calculateColor(selected, 0.75, Math.pow(0, 10)), new BasicStroke(stroke * 2), width, height, stroke, font, mean, yLower, mean, yUpper);
				drawLine(graphics, calculateColor(selected, 0.75, Math.pow(0, 10)), dashed, width, height, stroke, font, mean - stdev, yLower, mean - stdev, yUpper);
				drawLine(graphics, calculateColor(selected, 0.75, Math.pow(0, 10)), dashed, width, height, stroke, font, mean + stdev, yLower, mean + stdev, yUpper);
			...
			case MEDIAN:
				...
				drawLine(graphics, calculateColor(selected, 0.75, Math.pow(0, 10)), new BasicStroke(stroke * 2), width, height, stroke, font, median, yLower, median, yUpper);
				
				break;
			case MODE:
				double mode = getMode(selected);
				
				drawLine(graphics, calculateColor(selected, 0.75, Math.pow(0, 10)), new BasicStroke(stroke * 2), width, height, stroke, font, mode, yLower, mode, yUpper);
				
				break;
			case RMS:
				double rms = getRootMeanSquare(selected);
				
				drawLine(graphics, calculateColor(selected, 0.75, Math.pow(0, 10)), new BasicStroke(stroke * 2), width, height, stroke, font, rms, yLower, rms, yUpper);
